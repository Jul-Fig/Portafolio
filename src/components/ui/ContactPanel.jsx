      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 right-0 h-full w-full sm:max-w-md z-50 flex flex-col"
            style={{
          ...
          >
            <div className="flex items-center justify-between p-5 sm:p-6 pb-4"
              ...
              <div>
                <p className="text-xs font-semibold tracking-[0.25em] uppercase"
                  style={{ color: "#b48fe0", fontFamily: "'DM Mono',monospace" }}>Contacto</p>
            ...
            <div className="flex-1 overflow-y-auto p-5 sm:p-6">
              {status === "success"
                ...
                : <ContactForm form={form} status={status} error={error} onChange={handleChange} onSubmit={handleSubmit} />
              }
            </div>

            <div className="p-5 sm:p-6 pt-4" style={{ borderTop: "1px solid rgba(180,143,224,0.1)" }}>
              <p className="text-xs text-center" style={{ color: "#5a4a6a", fontFamily: "'DM Mono',monospace" }}>
                También puedes escribirme por{" "}
                <a href={SOCIAL_LINKS.linkedin} target="_blank" rel="noreferrer"
                  style={{ color: "#b48fe0" }} className="hover:underline">LinkedIn</a>
              </p>
            </div>
          </motion.aside>
        </>
...
const ContactForm = ({ form, status, error, onChange, onSubmit }) => (
  <form onSubmit={onSubmit} className="flex flex-col gap-4 sm:gap-5">
    ...
    {[
      ...
      <div key={name} className="flex flex-col gap-1.5">
        <label className="text-xs font-semibold tracking-widest uppercase"
          style={{ color: "#b48fe0", fontFamily: "'DM Mono',monospace" }}>{label}</label>
    ...
    <div className="flex flex-col gap-1.5">
      <label className="text-xs font-semibold tracking-widest uppercase"
        style={{ color: "#b48fe0", fontFamily: "'DM Mono',monospace" }}>Mensaje</label>
    ...
    <button type="submit" disabled={status === "loading"}
      ...
      style={{
        ...
        color: "#f0eaff", border: "1px solid rgba(180,143,224,0.3)",
