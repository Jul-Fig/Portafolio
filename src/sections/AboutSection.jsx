...
const AboutSection = () => (
  <section id="sobre-mi" className="relative w-full px-6 py-24 md:py-32 overflow-hidden">
    {/* Glow de fondo */}
    ...
    <div className="relative max-w-5xl mx-auto">
      ...
      <motion.h2 initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
        ...
        className="text-5xl md:text-7xl font-extrabold leading-none mb-10"
        style={{ fontFamily:"'Syne',sans-serif", color:"#f0eaff", letterSpacing:"-0.03em" }}>
        Desarrollador<br />
        <span style={{ WebkitTextStroke:"1.5px #b48fe0", color:"transparent" }}>Full Stack</span>
      </motion.h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        ...
        <motion.div initial={{ opacity:0, x:-30 }} whileInView={{ opacity:1, x:0 }}
          ...
          <p className="text-lg leading-relaxed mb-5"
            ...
            Soy <strong style={{ color:"#f0eaff", fontWeight:500 }}>Julian Figueredo</strong>,
            ...
            {STATS.map(({ value, label }) => (
              <div key={label}>
                <p className="text-3xl font-extrabold"
                  style={{ color:"#b48fe0", fontFamily:"'Syne',sans-serif" }}>{value}</p>
                <p className="text-xs tracking-widest uppercase mt-0.5"
                  style={{ color:"#7a6a8a", fontFamily:"'DM Mono',monospace" }}>{label}</p>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div variants={containerVariants} initial="hidden"
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {SKILLS.map(({ Icon, label }) => (
              <motion.div key={label} variants={itemVariants} whileHover={{ scale:1.05, y:-3 }}
                className="flex flex-col items-center gap-2 py-4 px-2 rounded-xl cursor-default transition-colors duration-200"
                ...
                <Icon style={{ color:"#b48fe0", fontSize:"1.7rem" }} />
                <span className="text-xs text-center"
                  style={{ color:"#9b8aad", fontFamily:"'DM Mono',monospace" }}>{label}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
